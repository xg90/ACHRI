...
# custom modification 1: widened the dominant-frequency search band from 1.6-3.0 Hz to 0.7-3.0 Hz
# and clamped the adaptive low cutoff to 0.5 Hz to reduce second-harmonic / diastolic double counting.
# custom modification 2: the exported CSV label column now stores the full source filename.
# custom modification 3: batch extraction now analyzes the full filtered waveform first, instead of
# restricting HeartPy to a cleaned or centered 30-second segment unless fallback is needed.
...
def _trim_initial_seconds(raw_signal, sample_rate=30.0):
...
def get_filtered_ppg_full(raw_signal, sample_rate=30.0):

	raw_signal, tElapsed = _trim_initial_seconds(raw_signal, sample_rate=sample_rate)
	...
	lowcut_ppg = max(0.5, max_power_freq_ppg - 0.8)
...
def get_filtered_ppg(raw_signal, sample_rate=30.0):
	filtered_PPG = get_filtered_ppg_full(raw_signal, sample_rate=sample_rate)
	filtered_PPG = get_clean_segment(filtered_PPG)
	return filtered_PPG


def get_center_window(signal, sample_rate=30.0, window_seconds=30.0):
	window_size = int(window_seconds * sample_rate)
	if len(signal) <= window_size:
		return signal

	start_indx = int((len(signal)//2) - (window_size//2))
	end_indx = start_indx + window_size
	return signal[start_indx:end_indx]


def run_heartpy_with_fallback(raw_signal, sample_rate=30.0):
	filtered_full = get_filtered_ppg_full(raw_signal, sample_rate=sample_rate)
	filtered_clean = get_clean_segment(filtered_full)
	min_len = max(24, int(sample_rate * 5))

	candidates = [('full_filtered', filtered_full)]
	if len(filtered_clean) >= min_len:
		candidates.append(('cleaned_segment_fallback', filtered_clean))
	candidates.append(('center_30s_fallback', get_center_window(filtered_full, sample_rate=sample_rate)))

	last_error = None
	for extraction_mode, candidate in candidates:
		if len(candidate) < min_len:
			continue
		try:
			wd, m = hp.process(candidate, sample_rate=sample_rate)
			return candidate, wd, m, extraction_mode
		except Exception as exc:
			last_error = exc

	if last_error is not None:
		raise last_error
	raise ValueError('Filtered PPG signal is too short after cleaning and fallback.')
...
def get_ppg_measures_batch(datapath, sample_rate=30.0):

	csv_fpath = os.path.join(datapath, 'PPG_features.csv')
	csvfile = open(csv_fpath, 'w', newline='')
	fp_writer = csv.writer(csvfile, delimiter=',')
	header = ['sub_id', 'label', 'bpm', 'ibi', 'sdnn', 'sdsd', 'rmssd', 'pnn20', 'pnn50', 'hr_mad', 'sd1', 'sd2', 's', 'sd1/sd2', 'breathingrate', 'extraction_mode']
	...
	for i in range(len(sub_ids)):
		...
		for fn in fn_list[i]:
			if os.path.splitext(fn)[-1] == '.csv' and fn.split('_')[0] == 'ppgSignal':
				...
				filepath = os.path.join(datapath, sub_id, fn)
				...
				vals.append(label)
				try:
					filtered, wd, m, extraction_mode = run_heartpy_with_fallback(raw_signal, sample_rate=sample_rate)
					for measure in m.keys():
						vals.append(str(m[measure]))
					vals.append(extraction_mode)
				except:
					extraction_mode = 'analysis_failed'
					vals = [sub_id, label] + ['']*13 + [extraction_mode]
					wd = {}
					m = {}
...
def load_dataframe(filepath):
	...
	params = ['sub_id', 'exertion_level', 'bpm', 'ibi', 'sdnn', 'sdsd', 'rmssd',
			'pnn20', 'pnn50', 'hr_mad', 'sd1', 'sd2', 's', 'sd1/sd2', 'breathingrate', 'extraction_mode']
